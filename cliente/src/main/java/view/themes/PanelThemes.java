...
public class PanelThemes extends JPanel {
    ...
    private void selectedCurrentLookAndFeel() {
        ...
        String theme = UIManager.getLookAndFeelDefaults().getString(Preferences.THEME_UI_KEY);
        ...
        for (int i = 0; i < themes.size(); i++) {
            ...
            if (theme != null && ti.resourceName != null && theme.substring(Preferences.RESOURCE_PREFIX.length()).equals(ti.resourceName)) {
                themesList.setSelectedIndex(i);
                break;
            }
        }
    ...
    private void setThemes(ThemesInfo themesInfo) {
        ...
        if (themesInfo.lafClassName != null) {
            ...
            String theme = UIManager.getLookAndFeelDefaults().getString(Preferences.THEME_UI_KEY);
            if (theme != null && themesInfo.resourceName.equals(theme.substring(Preferences.RESOURCE_PREFIX.length()))) {
                return;
            }
            FlatAnimatedLafChange.showSnapshot();
            IntelliJTheme.setup(getClass().getResourceAsStream(THEMES_PACKAGE + themesInfo.resourceName));
            Preferences.getState().put(Preferences.KEY_LAF_THEME, Preferences.RESOURCE_PREFIX + themesInfo.resourceName);
        }
        FlatLaf.updateUI();
