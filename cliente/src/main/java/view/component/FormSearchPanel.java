...
public class FormSearchPanel extends JPanel {
    ...
    private void init() {
        ...
        textSearch.putClientProperty(FlatClientProperties.TEXT_FIELD_LEADING_ICON, new FlatSVGIcon("demo/icons/search.svg", 0.4f));
    ...
    private void showRecentResult() {
        String[] recentSearch = Preferences.getRecentSearch(false);
        String[] favoriteSearch = Preferences.getRecentSearch(true);
        panelResult.removeAll();
        listItems.clear();
        if (recentSearch != null && recentSearch.length > 0) {
            ...
            if (!listItems.isEmpty()) {
                panelResult.add(createLabel("Recientes"), 0);
            }
        }

        if (favoriteSearch != null && favoriteSearch.length > 0) {
            panelResult.add(createLabel("Favoritos"));
    ...
    private Component createNoResult(String text) {
        ...
        JLabel label = new JLabel("Sin resultado para \"");
    ...
    private static class NoRecentResult extends JPanel {
        ...
        private void init() {
            ...
            JLabel label = new JLabel("Sin búsquedas recientes");
            label.putClientProperty(FlatClientProperties.STYLE, "" +
                    "foreground:$Label.disabledForeground;" +
                    "font:bold;");
            add(label);
    ...
    private class Item extends JButton {
        ...
        protected Component createRecentOption() {
            ...
            if (!isFavorite) {
                ...
                JLabel label = new JLabel(new SVGIconUIColor("demo/icons/favorite_filled.svg", 0.4f, "Component.accentColor", 0.8f));
                label.putClientProperty(FlatClientProperties.STYLE, "" +
                        "border:3,3,3,3;");
                panel.add(label);
            }
        ...
        private JButton createButton(String name, String icon, float scale, String hoverKey, float alpha) {
            SVGIconUIColor svgIcon = new SVGIconUIColor("demo/icons/" + icon, scale, "Label.disabledForeground", alpha);
